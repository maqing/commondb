...
public class RPTempTrackEntityAction extends TrackEntityAction 
implements Preparable, ServletRequestAware{
	...
	public void customizeAppendQuery() {
		...
		if ((rpDeviceID!=null)&&(rpDeviceID.length()>0)) {
			appendQueryStr = appendQueryStr + " and t_entity_" + String.valueOf(EntityDefine.RPTempMetaId) 
					+ "." + EntityDefine.RPTemp_DeviceID_CN + " = '" + rpDeviceID + "' ";
		}
		if ((rpBeginTime!=null)&&(rpBeginTime.length()>0)) {
			appendQueryStr = appendQueryStr + " and t_entity_" + String.valueOf(EntityDefine.RPTempMetaId) 
					+ "." + EntityDefine.RPTemp_Time_CN + " >= '" + rpBeginTime + "' ";
		}
		if ((rpEndTime!=null)&&(rpEndTime.length()>0)) {
			appendQueryStr = appendQueryStr  + " and t_entity_" + String.valueOf(EntityDefine.RPTempMetaId) 
					+ "." + EntityDefine.RPTemp_Time_CN + " <= '" + rpEndTime + "' ";
		}
