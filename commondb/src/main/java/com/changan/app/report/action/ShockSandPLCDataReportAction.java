...
public class ShockSandPLCDataReportAction extends JqGridBaseAction {  
	...
	public int getResultSize() {
		...
		ShocksanddeviceExample shockSandDeviceExample = new ShocksanddeviceExample();
		com.changan.plcinterface.model.ShocksanddeviceExample.Criteria shockSandCriteria= shockSandDeviceExample.createCriteria();
		if ((timePLCDataReportBegin!=null)&& (timePLCDataReportBegin.length()>0)) {
			if ((timePLCDataReportEnd!=null)&& (timePLCDataReportEnd.length()>0)) {
				shockSandCriteria.andPlctimeBetween(DateUtil.parseDate(timePLCDataReportBegin, "yyyy-MM-dd HH:mm:ss"), 
						DateUtil.parseDate(timePLCDataReportEnd, "yyyy-MM-dd HH:mm:ss"));
			} else {
				shockSandCriteria.andPlctimeGreaterThanOrEqualTo(DateUtil.parseDate(timePLCDataReportBegin, "yyyy-MM-dd HH:mm:ss"));
			}
		} else {
			if ((timePLCDataReportEnd!=null)&& (timePLCDataReportEnd.length()>0)) {
				shockSandCriteria.andPlctimeLessThanOrEqualTo(DateUtil.parseDate(timePLCDataReportEnd, "yyyy-MM-dd HH:mm:ss"));
			}
		}
		if ((deviceId!=null)&& (deviceId.length()>0)) {
			shockSandCriteria.andDeviceidEqualTo(Integer.parseInt(deviceId));
		}
		return localShockSanddeviceDAO.countByExample(shockSandDeviceExample);
	}  
	...
	public List listResults(int from, int length) {  
		...
		shockSandDeviceExample.setFrom(from);
		shockSandDeviceExample.setLength(length);
		shockSandDeviceExample.setOrderByClause(" plctime desc ");
		results = localShockSanddeviceDAO.selectByExamplePage(shockSandDeviceExample);
		return results;
	} 
	...
	public String execute() {
		...
		deviceIdResults = this.entityService.dynSelect( EntityDefine.ShockSandDevice_ID_CN , 
				"t_entity_" + EntityDefine.ShockSandDeviceMetaId, "id='" + entityId + "'");
		if (deviceIdResults.size()>0) {
			Map deviceIdMap = (Map) deviceIdResults.get(0);
			deviceId = (String) deviceIdMap.get(EntityDefine.ShockSandDevice_ID_CN);
		}
	...
	public String ExportToExcel() {
		return ExportToExcelFile(String.valueOf(EntityDefine.ShockSandDeviceMetaId),
	...
	public String ExportToExcelFile(String metaId, String device_ID_CN, String device_Name_CN) {
	    ...
	    String fileName = "震砂机采集PLC数据表.xls";
	    try {
	    ...
	    try {
           ...
           // wcf_left.setLocked(false) ;                                    // 设置锁定，还得设置SheetSettings启用保护和设置密码
			...
			rowIdx++;
   			...
   			ws.addCell(new jxl.write.Label(5, rowIdx, "锤击压力", wcf_header));    
   			ws.addCell(new jxl.write.Label(6, rowIdx, "锤击时间", wcf_header));    
   			ws.addCell(new jxl.write.Label(7, rowIdx, "震动时间", wcf_header));    
   			ws.addCell(new jxl.write.Label(8, rowIdx, "工序", wcf_header));    
   			ws.addCell(new jxl.write.Label(9, rowIdx, "加工工件编码", wcf_header));    
   			...
   			shockSandDeviceExample.setOrderByClause(" plctime desc ");
   			detailList = localShockSanddeviceDAO.selectByExample(shockSandDeviceExample);
   			
		    for(int i = 0 ; i < detailList.size() ; i ++) { 
		    	Shocksanddevice detailItem = (Shocksanddevice) detailList.get(i);
		    	rowIdx++;
		    	if (rowIdx>65530) {
	   			...
	   			ws.addCell(new jxl.write.Label(5, rowIdx, detailItem.getHammerpressure().toString(), wcf_center));    
	   			ws.addCell(new jxl.write.Label(6, rowIdx, detailItem.getHammertime().toString(), wcf_center));    
	   			ws.addCell(new jxl.write.Label(7, rowIdx, detailItem.getShocktime().toString(), wcf_center));    
	   			ws.addCell(new jxl.write.Label(8, rowIdx, detailItem.getProcessName(), wcf_center));    
	   			ws.addCell(new jxl.write.Label(9, rowIdx, detailItem.getWorkpiecename(), wcf_center)); 
		    }
