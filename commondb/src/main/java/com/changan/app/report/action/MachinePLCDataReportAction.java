...
public class MachinePLCDataReportAction extends JqGridBaseAction {  
	...
	public int getResultSize() {
		...
		MachinedeviceExample machineDeviceExample = new MachinedeviceExample();
		com.changan.plcinterface.model.MachinedeviceExample.Criteria machineCriteria= machineDeviceExample.createCriteria();
		if ((timePLCDataReportBegin!=null)&& (timePLCDataReportBegin.length()>0)) {
			if ((timePLCDataReportEnd!=null)&& (timePLCDataReportEnd.length()>0)) {
				machineCriteria.andPlctimeBetween(DateUtil.parseDate(timePLCDataReportBegin, "yyyy-MM-dd HH:mm:ss"), 
						DateUtil.parseDate(timePLCDataReportEnd, "yyyy-MM-dd HH:mm:ss"));
			} else {
				machineCriteria.andPlctimeGreaterThanOrEqualTo(DateUtil.parseDate(timePLCDataReportBegin, "yyyy-MM-dd HH:mm:ss"));
			}
		} else {
			if ((timePLCDataReportEnd!=null)&& (timePLCDataReportEnd.length()>0)) {
				machineCriteria.andPlctimeLessThanOrEqualTo(DateUtil.parseDate(timePLCDataReportEnd, "yyyy-MM-dd HH:mm:ss"));
			}
		}
		if ((deviceId!=null)&& (deviceId.length()>0)) {
			machineCriteria.andDeviceidEqualTo(Integer.parseInt(deviceId));
		}
		return localMachinedeviceDAO.countByExample(machineDeviceExample);
	}  
	...
	public List listResults(int from, int length) {  
		...
		machineDeviceExample.setFrom(from);
		machineDeviceExample.setLength(length);
		machineDeviceExample.setOrderByClause(" plctime desc ");
		results = localMachinedeviceDAO.selectByExamplePage(machineDeviceExample);
		return results;
	} 
	...
	public String execute() {
		...
		deviceIdResults = this.entityService.dynSelect( EntityDefine.MachineDevice_ID_CN , 
				"t_entity_" + EntityDefine.MachineDeviceMetaId, "id='" + entityId + "'");
		if (deviceIdResults.size()>0) {
			Map deviceIdMap = (Map) deviceIdResults.get(0);
			deviceId = (String) deviceIdMap.get(EntityDefine.MachineDevice_ID_CN);
		}
	...
	public String ExportToExcel() {
		return ExportToExcelFile(String.valueOf(EntityDefine.MachineDeviceMetaId),
	...
	public String ExportToExcelFile(String metaId, String device_ID_CN, String device_Name_CN) {
	    ...
	    String fileName = "加工中心采集PLC数据表.xls";
	    try {
	    ...
	    try {
           ...
           // wcf_left.setLocked(false) ;                                    // 设置锁定，还得设置SheetSettings启用保护和设置密码
			...
			rowIdx++;
   			...
   			ws.addCell(new jxl.write.Label(4, rowIdx, "错误代码", wcf_header));    
   			...
   			machineDeviceExample.setOrderByClause(" plctime desc ");
   			detailList = localMachinedeviceDAO.selectByExample(machineDeviceExample);
   			
		    for(int i = 0 ; i < detailList.size() ; i ++) { 
		    	Machinedevice detailItem = (Machinedevice) detailList.get(i);
		    	rowIdx++;
		    	if (rowIdx>65530) {
