...
function searchCharaWindow(metaId,dynStore){
	...
	if(!defStoreS || defStoreS == null){
		...
		defStoreS.on('load',function(s,records){
			...
			newGroup(charaIdArr,metaId,dynStore);
...
var searchCharaWin;
var groupSmS;
function newGroup(charaIdArr,metaId,dynStore){
	...
	if(!searchCharaWin || searchCharaWin == null){	
		searchCharaWin = new Ext.Window({ 
		    ...
		    buttons: [{
				            text:'搜索',
				            //handler:searchByChara(dynStore)
				            handler:function(){
						    	...
						    	dynStore.proxy.conn.url='admin/jsonEntityByChara.ac';
						    	dynStore.load({params: { metaId: in_metaId2,charaArr:charaArr }}); 
						        searchCharaWin.hide();
		    				}
					  }]
...
function searchByChara(dynStore)
{
	...
	//store2.proxy.conn.url='admin/jsonEntityByChara.ac?metaId='+in_metaId2+'&charaArr='+charaArr;
	//store2.load();
	dynStore.proxy.conn.url='admin/jsonEntityByChara.ac';
	dynStore.load({params: { metaId: in_metaId2,charaArr:charaArr }}); 
    searchCharaWin.hide();
