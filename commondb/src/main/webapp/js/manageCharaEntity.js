...
function rEntityChara(entityId){
	...
	if(!myDataStore || myDataStore == null){
		...
		myDataStore.on('load',function(s,records){
			var myDataIdArr=[];
			var MyCount=0;
			myDataStore.each(function(r){
				myDataIdArr[MyCount]=r.get('dataId');
				MyCount++;
			});
			selChara(myDataIdArr);
		});
	}
	myDataStore.load({params: { metaId: in_metaId,entityId:in_entityId }});
}

function selChara(myDataIdArr){
	var gSm=charaGroupGrid.getSelectionModel();
	gSm.clearSelections();
	if(myDataIdArr!=null&&myDataIdArr!=''){
		var recordArr = groupStore.queryBy(function(record,id){
	    	return myDataIdArr.indexOf(record.get('dataId'))!=-1;  
		},this).getRange();
...
function newCharaGroupGrid(metaId,response){
	...
	defStore = new Ext.data.JsonStore({
	    // store configs
	    autoDestroy: true,
	    //url: 'admin/listCharaDefByMeta.ac?metaId='+metaId,
	...
	defStore.on('load',function(s,records){
		...
		newGroupStore(in_charaIdArr,metaId,response);
	});
}	
...
function newGroupStore(charaIdArr,metaId,response){
	...
	var groupSm = new Ext.grid.CheckboxSelectionModel({singleSelect :false});
...
function newData(){
	...
	if(!newDataForm || newDataForm == null){	
		newDataForm=  new Ext.form.FormPanel({
		    id: 'newData_form',
		    url: 'admin/createCharaDataWithEntity.ac',
    ...
    if (Ext.getCmp('newData_form').form.isValid()){
        Ext.getCmp('newData_form').getForm().submit({
         success: function(f,a){
         	...
         	loadDefStore(in_metaId);
         	newDataWindow.hide();
         	//manageDataWindow.hide();
         },
         failure: function(f,a){
            Ext.Msg.alert('失败', '保存失败');
         },
         params : { action : 'submit',charaId:in_charaId,metaId:in_metaId,entityId:in_entityId}  
      });
   }
}
//更新data
    ...
    if (Ext.getCmp('editData_form').form.isValid()){
        Ext.getCmp('editData_form').getForm().submit({
         success: function(f,a){
         	...
         	loadDefStore(in_metaId);
            editDataWindow.hide();
            //groupStore.filter( 'charaId', 1, true, true );
            //manageDataWindow.hide();
         },
         failure: function(f,a){
            Ext.Msg.alert('失败', '保存失败');
...
function updateChara(){
    ...
    var conn = new Ext.data.Connection();
    conn.request({
                url : 'admin/updateREntityCharaDataArr.ac',
                params : {
                	metaId:in_metaId,
                ...
                success : function(resp, opt) {
                	//Ext.Msg.alert('成功','保存特征属性关系成功');
                },
                failure : function(resp, opt) {
                    Ext.Msg.alert('失败','保存特征属性关系失败');
                }
   });
}         