            ...
            for(var j = 0; j < config.picArray.length ; j++) {
                ...
                items.push({
		            ...
		            items:[{
                         columnWidth:.35,
                            xtype:'label',
                            text: config.picArray[j].attrName
		             ...
		             ,html:'<a target="_blank" id="dlink' + config.picArray[j].attrId + '" href="#" >下载</a>'
		             }]
		             }
		             }]
                });
    ...
    myInitFields: function(record,hieraData)
    {
        for(var j = 0; j < this.config.picArray.length ; j++) {
            ...
            if(record.data['p_' + this.config.picArray[j].attrId])
            {
                ...
                Ext.getDom('dlink' + this.config.picArray[j].attrId).href = '../' + record.data['p_' + this.config.picArray[j].attrId];
                
            }
            else
            {
                   Ext.getDom('image' + this.config.picArray[j].attrId).src = '../images/default.gif';  
                   Ext.getDom('dlink' + this.config.picArray[j].attrId).href = '../images/default.gif';  
            }
        }
           ...
           //清空所有节点
            ...
            if(checkedNodeArr )
            {
	            for(var k = 0 ; k < checkedNodeArr.length ; k ++)
	            {
	                ...
	                checkedNodeArr[k].attributes['keep'] = false;
	            }    
            }
            ...
            treepanel.getRootNode().cascade(function() { // descends into child nodes
			    if(this.attributes['keep'] != true) { // test this node
